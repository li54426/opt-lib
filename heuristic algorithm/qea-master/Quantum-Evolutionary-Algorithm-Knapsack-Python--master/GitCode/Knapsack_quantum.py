def Knapsack_quantum(agents,profits,weights,capacity,H,repair,penalty,iteration,teta):
    ...
    #------------------------------------------------------------------------------------
    def H_gate_min(v):
        min_vec = v[:] > np.arccos(np.sqrt(H))   # 开平方 np.arccos做反余弦
        min_vec_not = np.logical_not(min_vec)    # 相当于进入了非门
    ...
    # *************************************************************
    # ---------------------------------------------------------------------------------initialization
    local_group_size = 10  # 本地的种群大小是10
    local_migration_condition = 30   # 迁移的条件是30
    global_migration_condition = 70  # 全局的迁移的条件是70
    best_global_fitness = -math.inf  # 初始化的最佳的适应度，初始的时候是浮点数的无穷大，取负就是最小值
    tmp_size =  np.size(profits)     # 价值数组的大小
    ind_size = tmp_size              #  个体的大小                             # length of chromosom (each individual)
    iteration_fitness = convert_2_matrix([0 for x in range(iteration)])  # 个体适应度矩阵
    best_agent_fitness = convert_2_matrix([0 for x in range(agents)])    # 每次迭代产生的最佳的适应度                 # best agent fitness of each iteration
    ...
    for i in range(agents):
        # print(np.shape(p[i]))
        p[i] = Knapsack_observe(Q[i])                                   # 先随机的选择商品#set observe of Q(i) in P(i) array
        p[i] = Knapsack_repair(p[i],profits,weights,capacity,repair)    # 计算出当前选择的最佳的价值 #rapair for size of knapsack
        fitness[0,i] = Knapsack_fitness(p[i],profits,weights,capacity,penalty)     # 这里的适应度的计算其实就是计算出当前所选商品的价值的总和#calculate fitness of each P array
